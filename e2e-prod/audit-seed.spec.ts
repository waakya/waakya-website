...
const BIZ = "Sharma Interiors";
const PNG = Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==", "base64");
const PDF = Buffer.from("%PDF-1.4\n1 0 obj<<>>endobj\ntrailer<<>>\n%%EOF\n");

type Qa = { email: string; password: string; id?: string };
const owner: Qa = { email: `audit-owner-${RUN}@waakya.test`, password: randomBytes(18).toString("hex") };
const staff: Qa = { email: `audit-member-${RUN}@waakya.test`, password: randomBytes(18).toString("hex") };
...
test("02 owner creates a business, profile, invites staff; staff joins", async ({ browser }) => {
  shared.ownerCtx = await session(browser, owner, "Priya Sharma");
  ...
  await o.getByLabel("Name").fill("Rahul Verma");
  ...
  const { data } = await c.from("orgs").select("id, name").eq("name", BIZ).order("created_at", { ascending: false }).limit(1);
  expect(data?.length).toBe(1);
  shared.orgId = data![0].id;
  await o.goto(BASE + "/staff");
  await expect(o.getByText("Rahul Verma").first()).toBeVisible();
});
...
test("04 conversations: direct, message to task, group, attachment", async () => {
  ...
  await s.getByRole("button", { name: /Priya Sharma/ }).click();
  ...
  await o.getByRole("button", { name: "Rahul Verma" }).click();
...
test("07 attendance, leave with half day, holiday", async () => {
  ...
  const balanceRow = o.locator("li").filter({ hasText: "Rahul Verma" }).filter({ has: o.getByRole("button", { name: "1 day" }) });
...
test("99 save audit personas", async () => {
  const { writeFileSync } = await import("node:fs");
  const dir = process.env.AUDIT_DIR!;
  writeFileSync(`${dir}/creds.json`, JSON.stringify({ business: BIZ, owner: { email: owner.email, password: owner.password }, member: { email: staff.email, password: staff.password } }));
  await shared.ownerCtx!.storageState({ path: `${dir}/owner.state.json` });
  await shared.staffCtx!.storageState({ path: `${dir}/member.state.json` });
});
