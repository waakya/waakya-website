...
- **QA data on production:** removed. All 37 QA businesses and 103 `@waakya.test` accounts created by this pass and the earlier audit were deleted after testing (`e2e-prod/qa/cleanup.mjs`), along with their stored files. The five real businesses and every real account were untouched; the local test fixture "Waakya Test Co" (`owner@`/`staff@waakya.test`) is kept because the local suite signs in with it.
