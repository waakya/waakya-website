...
| B3 | Cloudflare R2 credentials (`R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`) | Proof photos in the intended production bucket. | Storage is behind `lib/storage`; without R2 it uses a **private** Supabase Storage bucket, which is correct but has a 1 GB ceiling. Switching is env-only. |
